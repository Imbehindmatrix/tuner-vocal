// --- OUTIL DE DEBUG (Affiche les erreurs sur l'écran du téléphone) ---
const debugConsole = document.getElementById('debug-console');
function log(msg) {
    console.log(msg);
    if(debugConsole) debugConsole.innerHTML += ">> " + msg + "<br>";
}

// Attrape les erreurs globales (syntaxe, crash)
window.onerror = function (msg, url, lineNo, columnNo, error) {
    log("ERREUR CRITIQUE: " + msg + " (Ligne: " + lineNo + ")");
    return false;
};
...
// --- ATTENTE DU CHARGEMENT DE LA PAGE (Sécurité DOM) ---
document.addEventListener('DOMContentLoaded', () => {
    log("Page chargée. Recherche des éléments...");

    const statusBox = document.getElementById('status-box');
    const bigNote = document.getElementById('big-note');
    const bigHz = document.getElementById('big-hz');
    const historyList = document.getElementById('history-list');
    const btnStart = document.getElementById('btn-start'); // On reprend votre ID btn-start
    const canvas = document.getElementById('spectrum-canvas');
    
    if (!btnStart) {
        log("ERREUR : Bouton 'btn-start' introuvable !");
        return;
    }
    
    const canvasCtx = canvas.getContext('2d');
    log("Système prêt. Appuyez sur le bouton.");

    // --- DÉMARRAGE ---
    btnStart.addEventListener('click', async () => {
        log("Clic reçu ! Tentative d'accès micro...");
        ...
        if (!AudioContext) {
            log("ERREUR : AudioContext non supporté.");
            alert("Votre navigateur ne supporte pas l'audio web.");
            return;
        }
        ...
        try {
            // Vérification HTTPS et API
            if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
                throw new Error("API Micro inaccessible. Êtes-vous en HTTPS ?");
            }
            ...
            // Gestionnaire spécial pour iOS (Safari préfère mp4/aac, Chrome préfère webm)
            try {
                recorder = new MediaRecorder(stream);
            } catch (e) {
                log("Essai format standard échoué, essai forcé...");
                // Fallback simple
                recorder = new MediaRecorder(stream); 
            }
    ...
    // --- OUTILS (Note, Autocorrelate, MP3) ---
    // ... Je garde vos fonctions exactes ici, elles sont incluses dans le bloc ...
